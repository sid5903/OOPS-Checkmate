                ...
                if (!chessBoard.hasChosenTile()) { // if there is no chosen piece . Then make this piece chosen...
                    ...
                    if (MoveUtilities.isValidMove(chessBoard, destinationTile)) {
                        ...
                        if (chosenTile != null && chosenTile.hasPiece() && chosenTile.getPiece().getType() == PieceTypes.PAWN) {
                            for (Move m : chosenTile.getPiece().availableMoves(chessBoard, chosenTile.getCoordinate())) {
                                if (m.isEnPassantMove() && m.getDestinationTile().getCoordinate().equals(destinationTile.getCoordinate())) {
                                    ...
                                    checkGameEnding(chessBoard, client);
                                    
                                    if (m.hasKilledPiece()) {
                                        if (m.getKilledPiece().getType() == PieceTypes.KING) {
                                            Team winnerTeam;
                                            winnerTeam = (m.getKilledPiece().getTeam() == Team.BLACK) ? Team.WHITE : Team.BLACK;
                        ...
                        if (!isCastling && !isEnPassant) {
                            ...
                            if (chosenTile.hasPiece() && chosenTile.getPiece().getType() == PieceTypes.PAWN) {
                                boolean isPromotionRank = (chosenTile.getPiece().getTeam() == Team.WHITE && destinationTile.getCoordinate().getY() == 0) || 
                                                        (chosenTile.getPiece().getTeam() == Team.BLACK && destinationTile.getCoordinate().getY() == 7);
                                if (isPromotionRank) {
                                    ...
                                    switch (choice) {
                                        case 0: promotionType = PieceTypes.QUEEN; break;
                                        case 1: promotionType = PieceTypes.ROOK; break;
                                        case 2: promotionType = PieceTypes.BISHOP; break;
                                        case 3: promotionType = PieceTypes.KNIGHT; break;
                                        default: promotionType = PieceTypes.QUEEN; break;
                                    }
